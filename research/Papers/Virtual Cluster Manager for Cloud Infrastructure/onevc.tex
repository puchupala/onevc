...
Section V demonstrate cases where VC manager performs better than existing utilities and their measurement.
...
Figure \ref{fig:onearch} architecture of show OpenNebula software stack.
...
OpenNebula maintain pool of host, disk image, network configuration, VM template and VM instance with onehost, oneimage, onevnet, onetemplate and onevm respectively.
To register configuration into each pool, user need to provide template to each utility accordingly (i.e. supply network configuration template to onevnet to add network configuration the pool) with the exception of onehost which accept parameters directly from the shell.
VM template could then refer registered resources (network configuration, disk image, etc.) to be used.

After all configurations are in place, to deploy VM, user could register VM template to template pool and use onetemplate utility to instantiate VM instance or just supply the template to onevm utility directly.
Figure \ref{fig:onevm} show deployment operation flow of onevm.
...
Figure \ref{fig:onevc} show VC deployment process of onevc.
