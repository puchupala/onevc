...
To address this problem, I propose virtual cluster manager, a unified utility for virtual cluster management.
...
In this research, I focus on Infrastructure as a Service\cite{ranabahu2009best} (IaaS) paradigm.
...
To simplify this process, I propose a utility which manage VC with existing VI toolkit.
...
In this research, I developed CLI Utility called onevc for OpenNebula 3.0 (VI toolkit) which provide the ability to manage VC as a whole.
...
Similar to how onevm works, to describe a VC to onevc utility, I use hierarchical template system.
...
In this research, I define VC with a hierarchical model.
...
To represent this VC with this model, I use 2 VM type to represent frontend VC and compute node VM.
...
To describe VC with my model, I use template.
...
% CHANGE TITLE AND REWRITE THIS PART
...
\section{Case Study}
Using the example VC template in Figure \ref{fig:template}, I deploy the VC with existing utility and onevc utility.

\subsection{With existing built-in utility}
Firstly, I created 2 templates for frontend and compute nodes and register it to onetemplate built-in utility with this command.

\texttt{\$ onetemplate create <path\_to\_templatefile>}

Then, I instantiate 1 frontend VM follow by a number of compute node.
To instantiate VM with onetemplate, I used this command.
...
It took 2 commands to register template and 1 command to instantiate each VM so I used 2+n commands in total.
I also need to maintain each template individually and make sure that configurations in both template worked together.
...
In total, I used 2 commands for automatic deployment and 3 commands for manual VM type deployment regardless of how many compute nodes are there in the cluster.
...
In order to flexibly describe VC, I use hierarchical VC model along with the template system.
...
With this architecture, I implemented onevc utility for OpenNebula and show how to deploy typical VC with this tool compare with existing built-in utility.
